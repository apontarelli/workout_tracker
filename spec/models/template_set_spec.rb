...
RSpec.describe TemplateSet, type: :model do
  let(:user) { User.create!(name: "John Doe", email: "john.doe@example.com", password: "password") }
  let(:template_workout) { TemplateWorkout.create!(user: user, name: "Morning Workout") }
  let(:exercise) { Exercise.create!(name: "Push Up") }
  let(:template_exercise) { TemplateExercise.create!(template_workout: template_workout, exercisable: exercise) }
