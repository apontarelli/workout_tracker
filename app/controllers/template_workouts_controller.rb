class TemplateWorkoutsController < ApplicationController
  ...
  before_action :set_template_workout, only: [:edit, :update, :destroy]
  ...
  def create
    ...
    if @template_workout.save
      redirect_to edit_program_template_workout_path(@program, @template_workout), notice: 'Program workout was successfully created.'
    else
  ...
  def update
    ...
    if @template_workout.update(template_workout_params)
      redirect_to edit_program_template_workout_path(@program, @template_workout), notice: 'Program workout was successfully updated.'
    else
  ...
  def template_workout_params
    params.require(:template_workout).permit(:name, template_exercises_attributes: [
      ...
      template_sets_attributes: [:id, :reps, :weight, :_destroy, :exercise_entry_id, :exercise_entry_type]
    ]).tap do |whitelisted|
      if whitelisted[:template_exercises_attributes]
        whitelisted[:template_exercises_attributes].each do |key, exercise|
